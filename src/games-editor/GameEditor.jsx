...
import {
  RequestKeys,
} from 'CourseAuthoring/editors/data/constants/requests';
import './GameEditor.scss';
import EditorContainer from 'CourseAuthoring/editors/containers/EditorContainer';
import SettingsOption from 'CourseAuthoring/editors/containers/ProblemEditor/components/EditProblemView/SettingsWidget/SettingsOption';
import Button from 'CourseAuthoring/editors/sharedComponents/Button';
import DraggableList, { SortableItem } from 'CourseAuthoring/generic/DraggableList';
...
export const GameEditor = ({
  ...
  // Custom save: the MFE's saveBlock games branch was removed, so persist via the
  // plugin's own save_settings thunk and close the editor on success.
  const handleSave = () => {
    if (!validateAllCards()) { return; }
    const content = hooks.getContent({ type, settings, list });
    saveGamesSettings({
...
export const mapDispatchToProps = {
  ...
  saveGamesSettings: thunkActions.game.saveGamesSettings,
};
